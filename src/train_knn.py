...
MODEL_OUTPUT_DIR = BASE_OUTPUT_DIR / "knn"
MODEL_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# =========================
# LOAD DATA
# =========================
X_train_full = np.load(BASE_OUTPUT_DIR / "X_train_pca.npy")
X_test = np.load(BASE_OUTPUT_DIR / "X_test_pca.npy")
y_train_full = np.load(BASE_OUTPUT_DIR / "y_train.npy")
...
for config in candidate_configs:
    ...
    print(f"Config: {config} -> Validation Accuracy: {val_acc:.4f}")
...
pd.DataFrame(search_results).to_csv(
    MODEL_OUTPUT_DIR / "knn_hyperparameter_search.csv",
    index=False
)
...
# =========================
final_model = KNeighborsClassifier(**best_config)
...
plt.ylim(0, 1)
plt.title("Per-Class F1 Score - KNN")
plt.ylabel("F1 Score")
...
# SAVE MODEL
# =========================
joblib.dump(final_model, MODEL_OUTPUT_DIR / "model.pkl")
